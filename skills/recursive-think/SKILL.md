...
final int MAX_DEPTH = 6;           // 深度软阈值：超过后倾向叶节点，但确需继续可突破
final int MAX_CORRECTION = 2;      // 同一子节点最多纠错轮数

// 总量硬闸（与 MAX_DEPTH 并存：深度是软提醒，总量是硬上限）——
// 每节点≥2 子在深树下会指数膨胀，MAX_DEPTH 只提醒不封顶，故补总节点数硬闸。
int MAX_TOTAL_NODES = 200;         // 整棵树节点总数硬上限，默认值；用户可在触发时覆盖
final int MAX_PARALLEL_SUBAGENTS = 8;  // 单次并行派发的 subagent 数上限（同一父节点子节点分批派）
...
void onCompactDetected() {
    ...
    if (wipDir != null) {
        ...
        reread(wipDir + "/budget.meta");                // 恢复总节点预算计数（见下）
    }
}

// ------------------------------------------------------------
// 总量预算 — 跨所有并行 subagent 共享的节点计数（存于 WIP 目录）
// ------------------------------------------------------------
// budget.meta 格式（JSON，UTF-8）：{"maxTotalNodes":200, "usedNodes":<已定稿节点数>}
// 由于子节点在独立上下文中并行运行，计数必须落盘共享，不能用内存变量。
// claimNodeBudget 在"确认要新增一个节点"时调用，原子地占用 1 个名额：
//   返回 true  → 名额充足，已占用，正常展开
//   返回 false → 已达 MAX_TOTAL_NODES 硬闸，调用方须停止拆分、按叶节点强制收敛
// 每个 recursiveUnit 实例在自己成为确定节点（叶或内部）时占用 1 个名额。
boolean claimNodeBudget(String wipDir) {
    // 原子读-改-写 budget.meta（并发写用文件锁/临时文件+rename 保证原子性）
    Budget b = readBudget(wipDir + "/budget.meta");
    if (b.usedNodes >= b.maxTotalNodes) {
        return false;                                   // 触顶，拒绝新增
    }
    b.usedNodes += 1;
    writeBudgetAtomic(wipDir + "/budget.meta", b);
    return true;
}
...
void main() {
    ...
    // 3. 自定义：低置信度或用户明确要求 → 走 buildCustomProfile() 问答流程（见其定义）
    ...
    // 初始化总节点预算（总量硬闸）。用户可在触发时覆盖 MAX_TOTAL_NODES；
    // 未指定则用默认值。计数落盘供所有并行 subagent 共享（见 claimNodeBudget）。
    if (userSpecifiedNodeBudget()) {
        MAX_TOTAL_NODES = userNodeBudget();
    }
    writeFile(wipDir + "budget.meta", {"maxTotalNodes": MAX_TOTAL_NODES, "usedNodes": 0});
    ...
    // 根节点也是内部节点，必须走完 recursiveUnit 全部步骤（含"整合 WIP"），
    // 之后才能 selfCheck。
    NodeResult rootResult = runAgent(recursiveUnit(rootProp, 0, "root", profile));
    // 此时 0-root-*.md 已含整棵树的层层汇总，只读这一个文件，不要跨层扫描。
...
NodeResult recursiveUnit(Proposition prop, int depth, String parentName, DomainProfile profile) {
    ...
    // ancestorKnowledge 中的知识，子节点可以复用（不重复搜索）、据此聚焦增量调研；
    // 发现祖先调研中的错误或过时信息时，通过纠错机制上报。

    // 步骤 1：调研 — 在祖先知识基础上增量调研
    Research research = doResearch(prop, profile, ancestorKnowledge);
    // 调研策略：先复用 ancestorKnowledge 已覆盖的知识，仅对祖先未覆盖的部分增量调研；
    // 调研方式由 profile.researchMethods 决定：
    ...
    // 仅当父节点要求交叉验证（prop.requiresCrossVerification，由 needsCrossVerification 决定）
    // 且调研方式包含 "web" 时执行
    ...
    // 消费者只有内部节点的子节点（判断在步骤 3），此处提前写以便步骤 5 直接引用
    // researchFilePath；若最终为叶节点则文件无害，随 WIP 目录清理。
    ...
    // 深度提醒：MAX_DEPTH 是提醒信号非硬上限。超过时倾向作为叶节点处理；
    // 但内容确需继续拆分（复杂、不可合并）时可继续——深度服从内容需要。
    if (depth >= MAX_DEPTH) {
    ...
    if (isLeaf(research, profile)) {
        ...
        // 总量记账：叶节点是终态节点，占用 1 个总量名额。
        // 叶节点已不再拆分，故此处只记账、不改行为——即使返回 false（预算已耗尽）
        // 也照常定稿本叶节点，仅让总计数如实反映（用于 selfCheck 的截断统计）。
        claimNodeBudget(wipDir);
        ...
        // 总量硬闸（在拆分前判定，与 MAX_DEPTH 软提醒并存）：
        // 尝试为"继续拆分"占用预算名额。已达 MAX_TOTAL_NODES 时 claimNodeBudget 返回 false，
        // 此时放弃拆分，把当前命题按叶节点强制收敛（走既有 forceFinalize/叶节点路径），
        // 并在输出中标注"因预算截断"。未触顶时行为与原来完全一致。
        if (!claimNodeBudget(wipDir)) {
            // 复用既有 forceFinalize（签名 (Proposition, ErrorReport) → NodeResult，
            // 与纠错上限分支同一调用）：用一个 errorType="预算截断" 的 ErrorReport 说明原因，
            // forceFinalize 会按其最低质量要求（写明假设、标注存疑、列待查项）定稿并写 WIP。
            ErrorReport truncReport = new ErrorReport(
                errorType:  "预算截断",
                detail:     "已达总节点预算上限 MAX_TOTAL_NODES=" + MAX_TOTAL_NODES + "，本命题停止拆分、强制收敛为叶节点",
                suggestion: null
            );
            NodeResult truncated = forceFinalize(prop, truncReport);
            truncated.isLeaf = true;                    // 强制收敛为叶节点，停止递归
            truncated.attributes.put("budgetTruncated", true);  // 供 writeOutput 标注"因预算截断"
            return truncated;
        }
        ...
        // 并行上限 MAX_PARALLEL_SUBAGENTS：子节点数超过上限时分批派发（每批 ≤ 上限，
        // 批间串行、批内并行），避免一次拉起过多 subagent。不改变结果，仅限制并发度。
        List<NodeResult> results = parallelDispatch(
            childProps,
            (p) -> runAgent(recursiveUnit(p, depth + 1, selfName, profile)),
            MAX_PARALLEL_SUBAGENTS
        );
        // 执行策略：同一父节点的子节点按 MAX_PARALLEL_SUBAGENTS 分批，每批在单条消息中发起多个 spawn_agent 调用
        ...
        // 整合 WIP（关键步骤 — 不可跳过）
        // 把子节点内容汇总进本节点 WIP，使父节点只需读本节点汇总文件（含根节点，
        // 汇总进 0-root-*.md 供 main() 的 selfCheck/writeOutput 消费）。跳过会迫使
        // 后续阶段跨层读全部文件，违背"每个节点独享完整上下文"。
        // 执行方式：读一个子节点 WIP、append、释放，逐个处理，避免同时驻留上下文。
...
boolean isLeaf(Research research, DomainProfile profile) {
    ...
    // recheck 是提醒重新审视，非机械循环或自动否决。智能体综合以上所有信号
    // （硬条件 + 领域信号 + 兄弟对比）自主定夺：该拆分返回 false，原子单元返回 true。
    return selfJudgment;
}
...
void writeOutput(Node tree, String topic, DomainProfile profile) {
    ...
    write(filename, [
        ...
        buildOverview(tree),               // ② 概览：节点总数、树深度、领域、主要分支概述
                                           //   若存在 budgetTruncated 节点，概览中注明
                                           //   "达总节点预算上限（MAX_TOTAL_NODES=N），部分分支因预算截断"
        buildASCIITree(tree),              // ③ 知识树 ASCII：标注 叶/内部、层级（截断节点额外标注 ✂ 预算截断）
        buildNodeDetails(tree),            // ④ 节点详情（深度优先先序遍历）
                                           //   attributes.budgetTruncated==true 的节点，末尾标注：
                                           //   > ✂ 因达总节点预算上限截断，本分支未继续展开
    ]);

    // 摘要（对话输出，不写入文档）
    print("文件路径、节点总数（叶/内部）、树深度、是否因预算截断");

    // 保留 WIP 目录，不自动删除（定位/语义见文末"WIP 目录的定位"）
    // if (writeSucceeded) {
    //     deleteDir(topic + "-wip/");
    // }
}
...
NodeResult runAgent(AgentTask task) {
    ...
    // 需要继续递归时通过 spawn_agent 派 subagent，
    // 每个 subagent 同样接收 SKILL 路径 + 自己的 context，执行 recursiveUnit。
}
...
// - 叶节点内容必须"充分展开"（详见步骤 4），充分利用其独享的完整上下文
