...
**生产环境**：禁止自动创建账号、自动赋权、自动清理任何数据。测试数据一律留下，在报告"遗留数据"段列清单（表名 + 主键 + 批次前缀），由用户决定处置。

**非生产环境（本地/测试）**：仅当**同时满足全部三条**才允许 `finally` 自动清理——① 数据是本脚本本批次自建；② 带批次前缀（`batch_id`）可精确圈定；③ 目标表在派单 prompt 给的**白名单**内。任一条不满足，同生产规则：留下 + 列清单。派单 prompt 未明确授权清理时，默认不清理。
...
try:
    ...
    # 仅当【非生产 + 本批次自建 + 批次前缀可圈定 + 表在白名单 + prompt 已授权】时才删：
    #   按 batch_id 前缀精确删除本批次数据，删后验证 DB 行数恢复到测试前基线
    # 否则：不删，把遗留数据写进报告"遗留数据"段（表 + 主键 + 批次前缀）交用户处置
    ...
```
