...
3. **数据纪律**：默认 agent **不得自行清理测试数据**，一律留下并在报告"遗留数据"段列清单由用户决定处置；生产环境更严——禁止自动建号/赋权/清理任何数据、禁真发外部消息、API-first。**唯一例外**：非生产环境（本地/测试）且同时满足①数据本批次自建 ②带批次前缀可精确圈定 ③目标表在本 prompt 给的白名单内 ④本 prompt 已明确授权清理——四条全中才允许 finally 按批次前缀删；缺一条即默认不清理
4. **引擎选择**：审查/测试/第二诊断可考虑 codex 引擎（避免自己写自己审的同源盲区，见 codex-delegate skill）
