...
| `~/.codex/skills/fact-checking` | `~/.claude/skills/fact-checking` | 同名；Claude 当前目录承接已并入的 gstack / source-verification 能力，S3 细查。 |
...
#### 用户已裁决并入 `fact-checking` 的目录

| Codex-only 目录 | 判定依据 | 后续动作 |
|---|---|---|
| `~/.codex/skills/gstack` | Claude 当前顶层目录已删除，且用户明确裁决其能力并入 `fact-checking` | 僵尸；后续删除独立目录，不逐项复审其子目录。 |
| `~/.codex/skills/source-verification` | Claude 当前顶层目录已删除，且用户明确裁决其能力并入 `fact-checking` | 僵尸；后续删除独立目录。 |

#### Claude 当前目录已移除的 10 个 skill

下列 10 项的分类依据统一为：**Claude 当前顶层目录已移除 + 用户已裁决这批官方 skill 移除**。S1 不读取正文重新论证。

| Codex-only 目录 | 分类结论 |
|---|---|
| `~/.codex/skills/algorithmic-art` | 僵尸 |
| `~/.codex/skills/amesh` | 僵尸 |
...
| `~/.claude/skills/docx` | Claude 当前新增/保留能力，Codex 无同名顶层目录 | 待 S3 判断是否应新增镜像；S1 不实现。 |
| `~/.claude/skills/pdf` | Claude 当前新增/保留能力，Codex 无同名顶层目录 | 待 S3 判断是否应新增镜像；S1 不实现。 |
| `~/.claude/skills/skill-creator` | Claude 当前新增/保留能力，Codex 无同名顶层目录 | 待 S3 判断是否应新增镜像或由 `.system` 等价能力承接；S1 不实现。 |
...
| Codex-only | 15 | 3 个 Codex 专属保留待升审；2 个并入 `fact-checking` 的僵尸；10 个用户裁决移除的僵尸。 |
| Claude-only | 5 | 2 个 Claude 专属不移植；3 个待 S3 判断是否新增镜像。 |

S1 到此只建立章节名、角色文件名、skill 目录名三级施工索引。S3 才逐一读取正文，处理镜像差异、Codex 工具与 Session 语义适配、入口文件去重和 Claude-only 能力是否已有 Codex 等价物；本表不提前作实现结论。
