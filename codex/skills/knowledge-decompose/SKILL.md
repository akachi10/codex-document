...
final int MAX_DEPTH = 6;           // 树深度上限——软提醒（非硬规则，触及仅重新审视，不强制截断）
...
// —— 硬预算上限（成本护栏，触顶即强制截断，见"预算护栏"小节）——
// 深度软、总量硬：MAX_DEPTH 触及只重新审视；下列两项触及必须强制收敛。
// 用户可在启动时覆盖默认值（A0 之后询问或由调用方传入）。
int MAX_TOTAL_NODES = 200;         // 整棵树节点总数硬上限（默认 200，用户可覆盖）
int MAX_PARALLEL_SUBAGENTS = 8;    // 单批并行 subagent 数硬上限（默认 8，用户可覆盖）
...
class CSVRow {
    ...
    boolean inCurrentBatch; // 是否被"当前训练轮次"选中，初始 false。
                            // 新轮次开始时：选中节点置 true、未选中置 false。
                            // 续训"上次未完成"判定 = inCurrentBatch && batch < currentBatch，
                            // 避免把本轮未选中的旧节点（batch 天然小于 currentBatch）误判为未完成。
...
void modeA() {
    // A0. 语言与地区 + Frontmatter 初始化
    Frontmatter fm = new Frontmatter();
    fm.language = ask("语言？zh/en/other");
    fm.region = ask("地区？none/us/uk/other");
    fm.generated = today();       // 生成日期 YYYY-MM-DD，由运行时提供

    // A1. 规模检查（目的：成本控制，递归拆解会派大量 subagent）
    String topic = extractTopicName();  // 从教材或用户输入提取
    fm.topic = topic;             // 主题名——writeOutput/writeProgressCSV 用它拼文件名，必须在输出前赋值

    // 版本号：首次拆解=1；再拆解（目标输出文件已存在）=旧版本+1。
    // 必须在拼出文件名（依赖 fm.topic）之后判定，故放在 A1 而非 A0。
    // 语义自洽：首次 version=1 → writeOutput 无同名文件、不备份，writeProgressCSV 无 CSV、首建；
    //           再拆解 version=old+1 → writeOutput 备份旧文件为 .v{old}.md，
    //           writeProgressCSV 因 csvVersion(=old) < fm.version(=old+1) 触发升级。
    String outFile = fm.topic + ".md";
    fm.version = exists(outFile) ? readVersion(outFile) + 1 : 1;
    ...
    // 此时 0-root-*.md 已含整棵树的层层汇总，只读这一个文件，不跨层扫描各 depth 文件。
...
// 预算护栏（硬上限）
// ============================================================
// 目的：递归拆解会派大量 subagent、生成大量节点，成本可能失控。用两道硬闸兜底。
// 与 MAX_DEPTH 的区别：MAX_DEPTH 是"软"提醒（触及只 recheck、可保留）；
// 本节两项是"硬"闸——触顶必须无条件收敛/限流，不给节点自主裁量。
//
// 1) MAX_TOTAL_NODES —— 整棵树节点总数硬上限
//    - 计数口径：globalNodeCount() = 已在 WIP 目录中定稿的节点文件数
//      （每个节点定稿时写一个 {depth}-{parentName}-{selfName}.md，故文件数 = 已生成节点数）。
//      WIP 目录是所有 subagent 都能观察的共享持久状态，用它做跨 subagent 的全局计数，
//      避免在分布式 subagent 间伪造原子内存计数。
//    - 触顶行为（见 recursiveUnit 步骤 5.0）：停止拆分，当前命题强制按叶节点收敛，
//      并在其 summary 标注"因预算截断"，writeOutput 在文档中提示该分支未继续拆解。
//
// 2) MAX_PARALLEL_SUBAGENTS —— 单批并行 subagent 数硬上限
//    - 触顶行为（见 recursiveUnit 步骤 5 的 parallelDispatch）：子节点数超上限时分批发起，
//      每批 ≤ 上限，批间串行、批内并行。不减少子节点数量，只限制同时在跑的 subagent 数。
//
// 两项默认值见常量区，用户可在启动时覆盖。未触顶时，本护栏不改变任何既有行为。

int globalNodeCount() {
    // 已定稿节点数 = WIP 目录中 {depth}-{parentName}-{selfName}.md 文件数（排除 *.research.md）
    return countWIPNodeFiles("{topic}-wip/");
}
...
//       togaf-wip/2-ADM-Phases-Preliminary.md  ← 嵌套节点（depth=2，父=ADM-Phases）
//
// 双向通道：
//   .research.md — 自顶向下：父节点调研后写入，子节点启动时读取，站在祖先肩膀上调研
//   .md          — 自底向上：子节点完成后写入，父节点汇总时读取
//
// 每个节点（含叶节点）完成后将完整结果写入对应 WIP 文件；父节点通过文件名模式
// "{depth+1}-{selfName}-*.md" 找到并读取所有子节点文件。根节点汇总完成后由
// writeOutput 合并为正式文档，WIP 目录保留不删（见 writeOutput 与文末 WIP 定位说明）。
...
NodeResult recursiveUnit(Proposition prop, int depth, String parentName) {
    ...
    // ancestorKnowledge 中的知识，子节点直接复用（术语/定义/概念关系不再重复搜索）、
    // 聚焦增量调研；发现祖先调研有错或过时信息时，通过纠错机制上报。

    // 步骤 1：调研 — 在祖先知识基础上增量调研
    Research research;
    if (prop.pageRange != null) {
        // 有教材：优先读教材对应页码（祖先已覆盖的可直接复用，只做增量）
        research = readPDF(prop.pageRange, ancestorKnowledge);  // 每次最多 PDF_PAGE_LIMIT 页
    } else {
        // 无教材：联网调研，优先官方文档和权威来源（仅对祖先未覆盖部分增量调研）
        research = searchWithCodexWeb(prop, ancestorKnowledge);
    }

    // 步骤 1.3：交叉验证 — 对关键事实进行独立信息源验证
    // 仅当父节点要求（prop.requiresCrossVerification，语义见该字段定义）时执行
    ...
    // 派一个 Codex subagent 专门做搜索验证，重点验证：
    //   - termEn / termCn 是否是该领域公认的标准术语
    //   - 术语是否有多种常见叫法（有则全部补入）
    //   - 命题范围与标准定义是否一致
    // 有教材时：搜索范围可以窄（主要验证术语标准性，不重复调研内容）
    // 无教材时：搜索范围宽（同时验证内容和术语）
    // subagent 使用 Codex 运行时默认模型，并通过 Codex web 搜索权威来源
    ...
    if (isLeaf(research)) {
        ...
        // 步骤 5.0：预算硬闸 —— 触及总节点数上限则停止拆分，当前命题强制按叶节点收敛
        // 深度软、总量硬：这里不是 recheck 提醒，而是无条件截断（与 MAX_DEPTH 的软提醒并存）。
        if (globalNodeCount() >= MAX_TOTAL_NODES) {
            Node truncatedLeaf = new Node();
            truncatedLeaf.isLeaf = true;
            fillAttributes(truncatedLeaf, research);
            truncatedLeaf.sampleQuestions = generateQuestions(truncatedLeaf);
            checkMediaRefs(truncatedLeaf, research);
            // 在 summary 标注截断，writeOutput 据此在文档中提示"因预算截断，本分支未继续拆解"
            truncatedLeaf.summary += "（⚠️ 因预算截断：达到总节点上限 " + MAX_TOTAL_NODES + "，本分支未继续拆解）";
            writeWIPFile(depth, parentName, truncatedLeaf.name, truncatedLeaf);
            return NodeResult.successMeta(truncatedLeaf);
        }
        ...
        // 并行度硬上限 MAX_PARALLEL_SUBAGENTS：子节点数超上限时分批发起，每批 ≤ 上限，
        // 批间串行、批内并行；不减少子节点数量，只限制同时在跑的 subagent 数。
        List<NodeResult> results = parallelDispatch(
            childProps,
            (p) -> runAgent(recursiveUnit(p, depth + 1, selfName)),
            MAX_PARALLEL_SUBAGENTS);
        // 执行策略：同一批的子节点在单条消息中发起多个 spawn_agent 调用
        ...
        //
        // 本节点把子节点内容汇总进自己的 WIP 文件，父节点只需读本节点汇总文件、
        // 不跨层读孙节点。层层向上，最终 0-root-*.md 含整棵树完整内容，成为
        // selfCheck 和 writeOutput 的数据来源。
        //
...
void writeOutput(Node tree, Frontmatter fm) {
    ...
    write(filename, [
        ...
        buildOverview(tree),                   // ② 概览：章节数、知识点总数、总学习时间、难度分布、前置知识
                                               //    若发生预算截断（存在 summary 含"因预算截断"的叶），
                                               //    概览额外提示："本次因达到总节点上限 MAX_TOTAL_NODES 截断，部分分支未继续拆解"
        ...
        buildNodeDetails(tree),                // ④ 节点详情（深度优先先序）
                                               ...
                                               //    > - {未验证事实1}（{note}）
                                               //    > - {未验证事实2}（{note}）
                                               //    因预算截断的叶：其 summary 已含"因预算截断"标注，原样透出
...
void modeC() {
    ...
    // 检查上次未完成
    // 判定条件：被上一轮选中（inCurrentBatch）且尚未答完（batch < currentBatch）。
    // 仅用 batch < currentBatch 会把"本轮未被选中的旧节点"误判进来，故加 inCurrentBatch 闸。
    List<Node> unfinished = findNodes(n -> n.csvRow.inCurrentBatch && n.csvRow.batch < currentBatch);
    ...
    // 标记本轮选中集合：选中置 inCurrentBatch=true，其余置 false（清掉上一轮标记）
    // 仅叶节点有 csvRow，故遍历 tree.allLeaves()
    for (Node n : tree.allLeaves()) { n.csvRow.inCurrentBatch = false; }
    for (Node n : scope)            { n.csvRow.inCurrentBatch = true;  }
...
NodeResult runAgent(AgentTask task) {
    ...
    // 拆解模式下通过 spawn_agent 派 Codex subagent，
    // 每个 subagent 同样接收 skill 路径 + 自己的 context，执行 runAgent。
    //
    // 术语验证 subagent（步骤 1.5）使用 Codex 运行时默认模型与 Codex web，
    // task 描述覆盖步骤 1.5 的四个验证项（termEn/termCn 标准性、其他叫法、范围一致），
...
// 通用规则
// ============================================================
// 出题通则（B/C 模式共用）：优先简答题（是否/选择也可）；语言与 frontmatter.language 一致；
// 评分依据 = 节点 summary + sampleQuestions 评分要点；每次出题前检查 last_question 确保角度不同。
//
// ── WIP 目录的定位（重要）──
