...
If the complexity check triggers (8+ files or 2+ new classes/services), STOP before any review-section work. Call request_user_input: name what's overbuilt, propose a minimal version that achieves the core goal, ask whether to reduce or proceed as-is. The request_user_input call is a tool_use, not prose — call the tool directly.

**STOP.** Do NOT proceed to Section 1 (Architecture review), edit the plan file with a proposed scope reduction, or call leave Plan mode until the user responds. Naming the 80% solution in chat prose and continuing — or loading the request_user_input schema via ToolSearch and then never invoking it — is the failure mode this gate exists to prevent.
...
> **STOP.** Before running the 4-section review, outside voice, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.codex/skills/gstack/plan-eng-review/sections/review-sections.md` and execute it
