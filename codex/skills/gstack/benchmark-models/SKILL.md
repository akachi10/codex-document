...
Use request_user_input with the preamble format:
...
If A: list top-level gstack skills that have SKILL.md files (from `find . -maxdepth 2 -name SKILL.md -not -path './.*'`), ask the user to pick one via a second request_user_input. Use the picked SKILL.md path as the prompt file.
...
If at least one is OK: request_user_input:
...
If judge is available, request_user_input:
