...
**STOP.** request_user_input once per issue. Recommend + WHY. Reference the persona.
...
**STOP.** request_user_input once per issue. Recommend + WHY.
...
**STOP.** request_user_input for any item that requires a design decision.
...
Follow the request_user_input format from the Preamble above. Additional rules for
DX reviews:

* **One issue = one request_user_input call.** Never combine multiple issues.
...
* **Zero findings:** if a section has zero findings, state "No issues, moving on"
  and proceed. Otherwise, use request_user_input for each gap — a gap with an
...
request_user_input. Never batch. For DX debt: missing error messages, unspecified upgrade
...
If any request_user_input goes unanswered, note here. Never silently default.
...
```bash
~/.codex/skills/gstack/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"PRODUCT_TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"COMPETITIVE_TIER","unresolved":N,"commit":"COMMIT"}'
```
...
eval "$(~/.codex/skills/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  ~/.codex/skills/gstack/bin/gstack-brain-cache invalidate developer-persona --project "$SLUG" 2>/dev/null || true
```
...
Use request_user_input with applicable options:
