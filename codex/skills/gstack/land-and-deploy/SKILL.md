...
to guide deploy verification. If nothing is detected, ask the user via request_user_input
...
Present the full dry-run results to the user via request_user_input:
...
4. If no deploy workflows detected and no URL provided: use request_user_input once:
...
If deploy fails (`conclusion` is `failure`): use request_user_input:
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use request_user_input:
