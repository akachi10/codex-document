...
2. **If missing, request_user_input** — let the user tell you or let gstack search the repo
...
When developing gstack, `.codex/skills/gstack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately**,
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using gstack concurrently.

**Check once per session:** Run `ls -la .codex/skills/gstack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all gstack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent gstack sessions
- During large refactors, remove the symlink (`rm .codex/skills/gstack`) so the
  global install at `~/.codex/skills/gstack/` is used instead
...
- **Engine + taxonomy:** `lib/redact-patterns.ts` (the single source of truth —
  ...
  internal + high-FP credential shapes that confirm via request_user_input, LOW =
...
When reviewing or merging community PRs, **always request_user_input** before accepting
any commit that:
...
categories require explicit user approval via request_user_input. No exceptions.
...
- No jargon: say "every question now tells you which project and branch you're in" not
  "request_user_input format standardized across skill templates via preamble resolver."
...
- Plan approvals, review outcomes (CEO / eng / design / outside-voice / codex findings),
  request_user_input decisions, scope negotiations.
...
- `cp design/dist/design ~/.codex/skills/gstack/design/dist/design`
