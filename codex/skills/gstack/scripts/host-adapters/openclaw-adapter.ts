 ...
 *
 * 1. request_user_input → prose instructions (tool call → "ask the user")
...
export function transform(content: string, _config: HostConfig): string {
  ...
  // 1. request_user_input references → prose
  result = result.replaceAll('request_user_input', 'ask the user directly in chat');
  result = result.replaceAll('Use request_user_input', 'Ask the user directly');
  result = result.replaceAll('use request_user_input', 'ask the user directly');
