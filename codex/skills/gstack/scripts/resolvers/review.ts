...
export function generateleave Plan modeGate(_ctx: TemplateContext): string {
...
export function generateAntiShortcutClause(_ctx: TemplateContext): string {
  return `**Anti-shortcut clause:** The plan file is the OUTPUT of the interactive review, not a substitute for it. Writing every finding into one plan write and calling leave Plan mode without firing request_user_input is the precise failure mode of the May 2026 transcript bug — the model explored, found issues, and dumped them into a deliverable rather than walking the user through them. If you have ANY non-trivial finding in any review section, the path from finding to leave Plan mode goes THROUGH request_user_input. Zero findings in every section is the only path to leave Plan mode that bypasses request_user_input. If you find yourself wanting to write a plan with findings before asking, stop and call request_user_input now — that's the bug, recognize it.`;
}
...
present the findings, then use request_user_input ONCE:
...
**Persist the result:**
\`\`\`bash
~/.codex/skills/gstack/bin/gstack-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
