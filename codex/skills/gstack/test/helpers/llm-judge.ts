 ...
 * regression scorer), and judgeRecommendation (request_user_input recommendation
 * substance scorer).
 *
...
/**
 * Score the quality of an request_user_input's recommendation line.
 *
 * Layered design:
...
export async function judgeRecommendation(askUserText: string): Promise<RecommendationScore> {
  ...
  const prompt = `You are scoring the quality of one specific line in an request_user_input: the "Recommendation: <choice> because <reason>" line. Score the because-clause substance on a 1-5 scale.
...
Surrounding request_user_input (context only — do NOT score this):
<<<UNTRUSTED_CONTEXT>>>
