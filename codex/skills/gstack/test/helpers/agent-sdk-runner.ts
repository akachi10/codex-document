...
export interface RunAgentSdkOptions {
   ...
   * to assert on request_user_input content silently pass without asserting.
   *
   * Callback contract matches the SDK: fires on every tool-use approval
   * request and on request_user_input invocations. For non-request_user_input
   * tools that tests don't care about, use `passThroughNonrequest_user_input`
   * to auto-allow them.
   */
...
/**
 * Pass-through helper: auto-allows any tool_use that isn't request_user_input.
 * Most plan-mode handshake tests only care about the handshake request_user_input;
 * every other tool (Read, Grep, Bash, Write, Edit, leave Plan mode) should just
 * run. Compose with a test-specific request_user_input handler:
 *
 ...
 *     }
 *     return passThroughNonrequest_user_input(toolName, input);
 *   }
 */
export function passThroughNonrequest_user_input(
  toolName: string,
...
export async function runAgentSdkTest(
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      // that want request_user_input interception without this flip would
      ...
      // When canUseTool is supplied, ensure request_user_input is in the allowed
      // tools list. Without it, Claude can't invoke request_user_input at all
      // and the callback never has a chance to fire on it.
      const baseTools = opts.allowedTools ?? ['Read', 'Glob', 'Grep', 'Bash'];
      const resolvedTools =
        hasCanUseTool && !baseTools.includes('request_user_input')
          ? [...baseTools, 'request_user_input']
          : baseTools;
