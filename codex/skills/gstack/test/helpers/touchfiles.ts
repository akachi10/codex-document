...
export const E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  // request_user_input-blocked regression case (--disallowedTools request_user_input
  ...
  // v1.21+ request_user_input-blocked regression tests — Conductor launches
  // claude with `--disallowedTools request_user_input --permission-mode default`
  // (verified via `ps`); skills must still surface user-decisions through a
  // fallback path (mcp__conductor__request_user_input or plan-file flow) rather
  ...
  // Per-finding request_user_input count + review-report-at-bottom assertion.
  ...
  // bug (model wrote a plan-mode plan and leave Plan mode'd without firing any
  // review-phase request_user_input). Uses runPlanSkillFloorCheck — minimal
  ...
  // request_user_input format regression (RECOMMENDATION + Completeness: N/10)
...
export const E2E_TIERS: Record<string, 'gate' | 'periodic'> = {
  ...
  'plan-ceo-mode-routing':     'periodic',   // ~$3/run, deep navigation through 8-12 prior request_user_inputs
  ...
  // request_user_input format regression — periodic (Opus 4.7 non-deterministic benchmark)
