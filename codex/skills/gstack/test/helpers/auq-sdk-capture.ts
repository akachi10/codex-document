/**
 * SDK-based AUQ capture — the reliable way to grade request_user_input content.
 *
 ...
 * the request_user_input it would have asked to a file. That captures exactly what
...
/**
 * Generic: drive ANY skill to its FIRST request_user_input and capture the
...
This is a capture test, not an interactive session. Skip any system-audit / environment-setup / codebase-exploration steps. When you reach the FIRST point where the skill would call request_user_input, write the verbatim full decision-brief text of that question (title, ELI10, stakes, recommendation, every option with its ✅/❌ pros/cons bullets, and the Net line) to ${outFile}. Do NOT call any tool to ask the user. Do NOT paraphrase. After writing the file, STOP.`;
 ...
 * the agent cannot wander to the global install). request_user_input is declared
...
export async function captureSectionReads(opts: {
  ...
  const skillPath = path.join(opts.planDir, opts.skillName, 'SKILL.md');
  const prompt = `You are running an automated skill-execution test. No human is present, so request_user_input is unavailable. The ONLY skill file you may read is this absolute path: ${skillPath}. Do NOT Glob/find/search for any other SKILL.md anywhere — especially nothing under ~/.claude or /Users.
...
- At any decision point that would call request_user_input, silently pick the skill's recommended option and continue. Do NOT stop to ask.
...
/**
 * Drive plan-ceo-review to its Step 0F mode-selection request_user_input in the
...
Proceed to Step 0F (Mode Selection), where the skill presents the 4 review-mode options to the user via request_user_input.

Write the verbatim text of that request_user_input (the full decision brief: title, ELI10, stakes, recommendation, every option with its pros/cons bullets, and the Net line) to ${outFile}. Do NOT call any tool to ask the user. Do NOT paraphrase. After writing the file, stop.`;
