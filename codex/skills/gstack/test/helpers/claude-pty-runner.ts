 ...
 * because plan mode doesn't use the request_user_input tool — it uses its
 * own TTY-rendered native confirmation UI.
 *
 * Why this exists: the SDK harness intercepts `canUseTool` for
 * `request_user_input`. Claude in plan mode renders its "Ready to execute"
 * confirmation as a native option list (1-4 numbered options) without
 * invoking the request_user_input tool. The SDK never sees it. Real PTY
 ...
 * when it short-circuits an request_user_input via the question-tuning resolver
 ...
 *
 * Used by v1.22 request_user_input-blocked regression tests to read the plan
 ...
 * supposed to gate on, even when request_user_input is --disallowedTools and
 ...
 * and `navigateToModerequest_user_input` in the routing test so tuning stays in sync.
 */
 ...
 * are NOT a skill's request_user_input — they're claude asking the user
 * whether to grant a tool/file permission. Tests that look for skill
 * request_user_inputs must explicitly skip these.
 *
...
/** Detect any request_user_input-shaped numbered option list with cursor. */
...
/**
 * Detect a prose-rendered request_user_input in plan mode.
 *
 * Plan-mode AUQs sometimes render as visible model output rather than via
 * the native numbered-prompt UI — e.g., when --disallowedTools request_user_input
...
export function parseNumberedOptions(
  ...
  // for the active request_user_input. Older numbered lists (e.g., a granted permission
 ...
 * to detect Step 0F mode-selection request_user_inputs, and by the upcoming
...
/**
 * Find the position of the first request_user_input-style numbered-option list
...
export function classifyVisible(
  ...
  opts?: {
     ...
     * request_user_input render as `wrote_findings_before_asking` rather than
  ...
  // (a numbered prompt means a permission/request_user_input is gating the write,
  ...
  while ((m = writeRe.exec(visible)) !== null) {
    ...
    if (opts?.strictPlanWrites && isPlanWrite) {
      if (auqRenderIdx < 0 || writePos < auqRenderIdx) {
        return {
          ...
          summary: `Write/Edit to ${target} fired before any request_user_input render`,
        };
      }
    ...
    if (!sanctioned && !isNumberedOptionListVisible(visible)) {
      return {
        ...
        summary: `Write/Edit to ${target} fired before any request_user_input`,
      };
    }
  }
  ...
  if (isAutoDecidedVisible(visible)) {
    return {
      ...
      summary:
        'skill auto-decided an request_user_input via the AUTO_DECIDE preamble (the user never saw the prompt)',
    };
  }
  ...
  if (isNumberedOptionListVisible(visible)) {
    ...
    return {
      outcome: 'asked',
      summary: 'skill fired a numbered-option prompt (request_user_input or routing-injection)',
    };
  }
  // Prose-rendered AUQ: model surfaced the question as lettered or numbered
  // options in plain text (typical under --disallowedTools request_user_input
  ...
  if (isProseAUQVisible(visible)) {
    ...
    return {
      outcome: 'asked',
      summary: 'skill rendered a prose-style request_user_input (model waiting for user input)',
    };
  }
  return null;
}
...
// ────────────────────────────────────────────────────────────────────────────
// Per-finding request_user_input count primitives (used by runPlanSkillCounting).
//
...
/**
 * Captured identity of an request_user_input — the rendered question text plus
 ...
 */
export interface request_user_inputFingerprint {
...
export type Step0BoundaryPredicate = (
  answeredFingerprint: request_user_inputFingerprint,
) => boolean;
...
/**
 * Stable hash for an request_user_input's identity — combines normalized prompt
...
export interface PlanSkillObservation {
   ...
   *                     request_user_input or the routing-injection prompt)
   ...
   * v1.22 request_user_input-blocked regression tests verify the plan file
   ...
   * prose-rendered request_user_input (lettered or numbered options visible)
   ...
   * by the time outcome=plan_ready fires, the leave Plan mode "Ready to
 ...
 *     without firing request_user_input because they short-circuit.
 *
 ...
 * the request_user_input tool — so canUseTool never fired and the assertion
...
export async function runPlanSkillObservation(opts: {
   ...
   *  request_user_input-blocked regression tests to pass
   *  `['--disallowedTools', 'request_user_input']` (the flag set Conductor
   *  uses to remove native request_user_input in favor of its MCP variant).
   ...
   * rendered request_user_input list).
   */
  env?: Record<string, string>;
  /**
   ...
   * request_user_input or fall back to a Decisions section. Without this,
...
// counts distinct review-phase request_user_input fingerprints. The actual
...
export interface PlanSkillCountObservation {
  ...
  /** Wall time (ms) until the outcome was decided. */
  elapsedMs: number;
  /** All distinct request_user_inputs observed, in observation order. */
  fingerprints: request_user_inputFingerprint[];
 ...
 * request_user_inputs until a terminal signal fires.
 *
 * Flow:
...
export async function runPlanSkillCounting(opts: {
   ...
   */
  firstAUQPick?: (fp: request_user_inputFingerprint) => number;
  ...
  const fingerprints: request_user_inputFingerprint[] = [];
    ...
    while (Date.now() - budgetStart < timeoutMs) {
      ...
      while ((m = writeRe.exec(visible)) !== null) {
        ...
        if (!sanctioned && !isNumberedOptionListVisible(visible)) {
          return snapshot(
            'silent_write',
            `Write/Edit to ${target} fired before any request_user_input`,
            visible,
          );
        }
      }
      ...
      const fp: request_user_inputFingerprint = {
...
// ────────────────────────────────────────────────────────────────────────────
// runPlanSkillFloorCheck — minimal "did the agent fire ANY request_user_input?"
// observer for gate-tier floor tests catching the May 2026 transcript bug
// (model wrote plan + leave Plan mode'd with reviewCount=0).
//
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      if (
        ...
        return {
          ...
          summary: 'agent rendered an request_user_input (floor met)',
          evidence: visible.slice(-3000),
          elapsedMs: Date.now() - startedAt,
        };
      }
      ...
      if (isPlanReadyVisible(visible)) {
        return {
          auqObserved: false,
          outcome: 'plan_ready',
          summary: 'agent reached plan_ready without firing any request_user_input',
          evidence: visible.slice(-3000),
          elapsedMs: Date.now() - startedAt,
        };
      }
