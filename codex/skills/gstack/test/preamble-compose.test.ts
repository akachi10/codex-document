 ...
 *
 * Asserts that the request_user_input Format section renders BEFORE the
...
describe('Preamble composition order', () => {
  test('request_user_input Format renders before Model-Specific Behavioral Patch (tier 2, claude)', () => {
  ...
  test('request_user_input Format renders before Model-Specific Behavioral Patch (tier 2, opus-4-7)', () => {
  ...
  test('request_user_input Format renders before Model-Specific Behavioral Patch (tier 3)', () => {
  ...
  test('tier 1 preamble does NOT include request_user_input Format (but MAY include overlay)', () => {
    const out = generatePreamble(makeCtx('claude', 1));
    expect(out).not.toContain('## request_user_input Format');
  });
});
