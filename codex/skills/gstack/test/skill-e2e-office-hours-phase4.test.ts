 ...
 * request_user_input. The fix is the STOP gate added to office-hours/SKILL.md.tmpl
 * Phase 4 footer.
 *
 ...
 * captureInstruction tells the agent to dump the verbatim Phase 4 request_user_input
...
// question. Without this, a captured request_user_input from an earlier phase
// would false-pass.
...
`);
  ...
  // Extract only the request_user_input Format spec + Phase 4 section from
  ...
  const fmtStart = fullSkill.indexOf('## request_user_input Format');
...
function captureInstruction(outFile: string): string {
  return `Write the verbatim text of the Phase 4 (Alternatives Generation) request_user_input you would have made to ${outFile} (full text including all option labels, recommendation line with because-clause, and net-line). Do NOT call any tool to ask the user. Do NOT paraphrase — include the exact prose you would have shown. This is a format-capture test, not an interactive session.`;
}

describeIfSelected('Office Hours Phase 4 — Architectural fork must surface request_user_input', ['office-hours-phase4-fork'], () => {
  let workDir: string;
  let outFile: string;
...
After writing the file with that ONE Phase 4 question, stop. Do not continue to Phase 4.5 or Phase 5.`,
    ...
    await assertRecommendationQuality({
      ...
      evalTitle: 'Office Hours Phase 4 — Architectural fork must surface request_user_input',
      result,
