 ...
 *
 * 1. Format positive — every request_user_input renders with D<N> / ELI10 /
 ...
 * request_user_input text to $OUT_FILE; regex predicates run on the captured
...
function captureInstruction(outFile: string): string {
  return `Write the verbatim text of the single request_user_input you would have made to ${outFile} (full text including D<N> header, ELI10, Stakes, Recommendation, Pros/cons, and Net line — the complete rich markdown body). Do NOT call any tool to ask the user. Do NOT paraphrase. This is a format-capture test.`;
}
...
Read plan.md — two cache approaches with real tradeoffs. Pick the architectural approach via request_user_input (Step 0C-bis / Implementation Alternatives). These options differ in coverage.
...
Read plan.md — this has REAL tradeoffs between Redis and in-memory caching (both have pros and cons). Pick the architectural approach via request_user_input.
...
Read plan.md — Option A dominates Option B on coverage. This is NOT a taste call. Pick the approach via request_user_input (Step 0C-bis / Implementation Alternatives — coverage-differentiated, so Completeness: N/10 applies).
...
Read plan.md — this is a destructive one-way action (terminate all sessions). Ask the user to confirm via request_user_input. This is a legitimate hard-stop choice — the hard-stop escape (\`✅ No cons — this is a hard-stop choice\`) is allowed here because there is no meaningful alternative besides doing or not doing the action.
