 ...
 * `sections/*.md` could strand the request_user_input decision-brief format (or a
 ...
 *      `## request_user_input Format` block, i.e. preamble tier >= 2) carries the
...
/** Mandatory elements of the request_user_input decision-brief format. Each is a
...
const MANDATORY: Array<{ name: string; re: RegExp }> = [
  { name: '## request_user_input Format header', re: /##\s*request_user_input Format/i },
  ...
  { name: 'AUQ-failure fallback subsection', re: /When request_user_input is unavailable or a call fails/i },
...
const PER_SKILL_RULES: Record<string, RegExp[]> = {
  'plan-ceo-review': [/One issue = one request_user_input call/i],
  'plan-eng-review': [/One issue = one request_user_input call/i],
  'plan-design-review': [/One issue = one request_user_input call/i],
  'plan-devex-review': [/One issue = one request_user_input call/i],
...
const interactive = skills.filter(s =>
  /##\s*request_user_input Format/i.test(fs.readFileSync(s.skillMd, 'utf-8')),
);
...
describe('AUQ format is always-loaded (token-reduction safety net)', () => {
  ...
  test('every expected interactive skill still ships the AUQ format block', () => {
    ...
    if (missing.length > 0) {
      throw new Error(
        `These skills lost their always-loaded request_user_input format block ` +
  ...
  for (const { skill, skillMd, sectionsDir } of skills.filter(s => s.sectionsDir)) {
    test(`${skill} (carved): AUQ format block lives in the skeleton, not only sections/`, () => {
      const body = fs.readFileSync(skillMd, 'utf-8');
      expect(body).toMatch(/##\s*request_user_input Format/i);
