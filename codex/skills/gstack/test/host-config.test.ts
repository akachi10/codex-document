...
describe('validateAllConfigs', () => {
  ...
  test('duplicate globalRoot detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.dup', globalRoot: '.codex/skills/gstack' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    expect(HOST_PATHS.claude.skillRoot).toBe('~/.codex/skills/gstack');
    expect(HOST_PATHS.claude.binDir).toBe('~/.codex/skills/gstack/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.codex/skills/gstack/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.codex/skills/gstack/design/dist');
  });
