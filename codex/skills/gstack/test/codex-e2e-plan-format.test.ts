 ...
 * line on request_user_input calls. The user has to manually re-prompt "ELI10 and don't
 ...
 *
 * Assertions on captured request_user_input text:
...
const ELI10_LENGTH_FLOOR = 400; // full request_user_input content should be at least this long
...
describeCodex('Codex Plan Format — CEO Mode Selection', () => {
  ...
  testIfSelected('codex-plan-ceo-format-mode', async () => {
    const result = await runCodexSkill({
      skillDir,
      prompt: `Read the plan-ceo-review skill. Read plan.md (the plan to review). Proceed to Step 0F (Mode Selection) where the skill presents 4 mode options (SCOPE EXPANSION, SELECTIVE EXPANSION, HOLD SCOPE, SCOPE REDUCTION) via request_user_input. These options differ in kind (review posture), not coverage. ${captureInstruction(outFile)}`,
      timeoutMs: 300_000,
      cwd: planDir,
...
describeCodex('Codex Plan Format — CEO Approach Menu', () => {
  ...
  testIfSelected('codex-plan-ceo-format-approach', async () => {
    const result = await runCodexSkill({
      skillDir,
      prompt: `Read the plan-ceo-review skill. Read plan.md. Proceed to Step 0C-bis (Implementation Alternatives / Approach Menu) where the skill generates 2-3 approaches (minimal viable vs ideal architecture) and presents them via request_user_input. These options differ in coverage so Completeness: N/10 applies. ${captureInstruction(outFile)}`,
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'gstack-plan-ceo-review',
      sandbox: 'workspace-write',
...
describeCodex('Codex Plan Format — Eng Coverage Issue', () => {
  ...
  testIfSelected('codex-plan-eng-format-coverage', async () => {
    const result = await runCodexSkill({
      skillDir,
      prompt: `Read the plan-eng-review skill. Read plan.md. In your Section 3 Test Review, generate ONE request_user_input about test coverage depth where options are clearly coverage-differentiated: A) full coverage incl. edge + error paths (Completeness 10/10), B) happy path only (7/10), C) smoke test (3/10). ${captureInstruction(outFile)}`,
      timeoutMs: 300_000,
      cwd: planDir,
...
describeCodex('Codex Plan Format — Eng Kind Issue', () => {
  ...
  testIfSelected('codex-plan-eng-format-kind', async () => {
    const result = await runCodexSkill({
      skillDir,
      prompt: `Read the plan-eng-review skill. Read plan.md. In your Section 1 Architecture review, generate ONE request_user_input about an architectural choice where the options differ in kind (e.g. Redis vs Postgres materialized view vs in-process cache — different kinds of systems with different tradeoffs, NOT more-or-less-complete versions of the same thing). ${captureInstruction(outFile)}`,
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'gstack-plan-eng-review',
      sandbox: 'workspace-write',
