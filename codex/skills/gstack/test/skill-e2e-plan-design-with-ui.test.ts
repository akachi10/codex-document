 ...
 * must reach a real skill numbered-option request_user_input (its first design-rating
...
describeE2E('/plan-design-review with UI scope (gate)', () => {
  test(
    'reaches a real skill request_user_input (or plan_ready) without echoing the no-UI early-exit phrase',
    async () => {
      ...
      try {
        ...
        while (Date.now() - start < budgetMs) {
          ...
          // Real skill request_user_input visible (not a permission dialog)?
          if (
