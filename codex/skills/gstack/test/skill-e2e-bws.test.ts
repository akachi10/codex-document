...
Log the operational learning now. Then say what you logged.`,
  ...
  testConcurrentIfSelected('session-awareness', async () => {
    ...
    // Extract request_user_input format instructions from a generated SKILL.md.
    // ROOT/SKILL.md is the browse skill (Tier 1) and does NOT contain the
    // "## request_user_input Format" section — that block is only emitted for
    ...
    if (!skillMd.includes('## request_user_input Format')) {
      ...
      for (const candidate of skillDirs) {
        ...
        if (content.includes('## request_user_input Format')) {
          skillMd = content;
