...
describe('gen-skill-docs', () => {
  ...
  test('tier 2+ skills contain ELI10 simplification rules (request_user_input format)', () => {
    // Root SKILL.md is tier 1 (no request_user_input format). Check a tier 2+ skill instead.
  ...
  test('tier 1 skills do NOT contain request_user_input format', () => {
    ...
    expect(content).not.toContain('## request_user_input Format');
  ...
  test('plan-review generated preambles stay under the Option A budget', () => {
    ...
    // \\u-escape CJK rule (rule 12 + self-check item) to the request_user_input
    // preamble.
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer as neutral forward reference to EXIT PLAN MODE GATE', () => {
    ...
    expect(content).toContain('GSTACK REVIEW REPORT');
    expect(content).toContain('leave Plan mode');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  ...
  test('contains opt-in request_user_input text', () => {
...
describe('INVOKE_SKILL resolver', () => {
  ...
  test('INVOKE_SKILL output includes default skip list', () => {
    ...
    expect(ceoContent).toContain('request_user_input Format');
  });
...
describe('preamble routing injection', () => {
  ...
  test('preamble includes routing injection request_user_input', () => {
...
describe('DESIGN_SKETCH extended with outside voices', () => {
  ...
  test('offers opt-in via request_user_input', () => {
...
describe('Codex generation (--host codex)', () => {
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.codex/skills/gstack → $GSTACK_ROOT
    expect(content).not.toContain('~/.codex/skills/gstack');
    expect(content).toContain('$GSTACK_ROOT');

    // Rule 2: .codex/skills/gstack → .agents/skills/gstack
    expect(content).not.toContain('.codex/skills/gstack');
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .codex/skills/gstack symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  test('learnings search includes request_user_input for first-time cross-project opt-in', () => {
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
...
describe('plan-mode-info resolver (handshake-replacement)', () => {
  ...
  test('0C-bis STOP block present in plan-ceo-review/SKILL.md', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-ceo-review', 'SKILL.md'), 'utf-8');
    const presentIdx = content.indexOf('Present these approach options via request_user_input');
...
// request_user_inputs to reach the report-write step, which the harness
...
describe('EXIT PLAN MODE GATE placement', () => {
  ...
  test('gate is the terminal ## heading in every plan-* review SKILL.md', () => {
    for (const skill of planSkills) {
      ...
      expect(md, `${skill}/SKILL.md gate body`).toContain('Failing this gate and calling leave Plan mode anyway is a contract violation');
    }
  });
  ...
  test('codex/SKILL.md contains gate (mid-file per D5; Step 2B/2C follow)', () => {
    ...
    expect(codex).toContain('Failing this gate and calling leave Plan mode anyway is a contract violation');
  });
});
