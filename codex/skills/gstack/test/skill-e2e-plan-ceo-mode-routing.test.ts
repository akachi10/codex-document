 ...
 * request_user_input and the user picks HOLD SCOPE or SCOPE EXPANSION,
 ...
 *
 * Tier: periodic (not gate). Each run navigates 8-12 prior request_user_inputs (telemetry,
 * proactive, routing, vendoring, brain, office-hours, premise×3, approach)
 * before reaching Step 0F. At ~30s per request_user_input that's a 4-6 min navigation
...
/**
 * Navigate prior request_user_inputs by picking option 1 until we hit an request_user_input whose
 * options match one of the 4 mode names. Returns the option index
 * matching `targetMode`, with the buffer marker pointing AT that request_user_input.
 *
 * Throws if we don't reach the mode request_user_input within `maxNav` prior request_user_inputs or
 * the overall budget.
 */
async function navigateToModerequest_user_input(
  session: ClaudePtySession,
  since: number,
  targetMode: ModeCase['mode'],
  opts: { maxNav?: number; budgetMs?: number } = {},
): Promise<{ modeIndex: number; visibleAtMode: string }> {
  // /plan-ceo-review's mode request_user_input (Step 0F) sits behind several preamble
  ...
  while (Date.now() - start < budgetMs) {
    ...
    // Is THIS the mode request_user_input?
    if (opts.some(o => MODE_RE.test(o.label))) {
      const target = opts.find(o => o.label.toUpperCase().includes(targetMode));
      if (!target) {
        throw new Error(
          `Mode request_user_input rendered but target "${targetMode}" not in option labels:\n` +
    ...
    // Not the mode request_user_input — answer with option 1 (recommended) and continue.
    if (priorAnswered >= maxNav) {
      throw new Error(
        `Navigated ${maxNav} prior request_user_inputs without reaching the mode request_user_input. ` +
        `Last list:\n${opts.map(o => `  ${o.index}. ${o.label}`).join('\n')}`,
      );
    }
    priorAnswered++;
  ...
  throw new Error(`Mode request_user_input not reached within ${budgetMs}ms`);
}
    ...
    test(
      ...
      async () => {
        ...
        try {
          ...
          const { modeIndex } = await navigateToModerequest_user_input(session, since, c.mode);
          ...
          // Wait for downstream evidence: either next request_user_input or plan_ready or
          ...
          while (Date.now() - start < budgetMs) {
            ...
            if (
              ...
              // Plan-ready AND a follow-up request_user_input are both visible but
