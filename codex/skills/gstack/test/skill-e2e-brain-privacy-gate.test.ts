 ...
 * every tool use via canUseTool, and asserts: one of the request_user_inputs
...
import { runAgentSdkTest, passThroughNonrequest_user_input, resolveClaudeBinary } from './helpers/agent-sdk-runner';

const shouldRun = !!process.env.EVALS && process.env.EVALS_TIER === 'periodic';
const describeE2E = shouldRun ? describe : describe.skip;

describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('gstack skill preamble fires the 3-option request_user_input when gbrain is detected', async () => {
    ...
    try {
      ...
      await runAgentSdkTest({
        ...
        userPrompt:
          `Read the skill file at ${learnSkill} and follow its instructions from the top, including every preamble directive. Execute every bash block. If any request_user_input fires, present it.`,
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    try {
      ...
      // No request_user_input should have matched the privacy gate's prose.
      const privacyQuestions = askUserQuestions.filter((aq) => {
        const qs = aq.input.questions as Array<{ question: string }>;
        return qs.some(
          (q) =>
