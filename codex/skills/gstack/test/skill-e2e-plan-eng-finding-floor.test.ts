/**
 * /plan-eng-review request_user_input floor regression (gate, paid, real-PTY).
 *
 * Catches the May 2026 transcript bug where /plan-eng-review wrote a
 * multi-section review plan to ~/.claude/plans/ and called leave Plan mode
 * without firing any request_user_input. See
...
describeE2E('/plan-eng-review request_user_input floor (gate)', () => {
  test(
    'seeded forcing finding causes the agent to fire at least one request_user_input',
    async () => {
      ...
      if (obs.outcome !== 'auq_observed') {
        throw new Error(
          `floor test FAILED: outcome=${obs.outcome} elapsed=${obs.elapsedMs}ms\n` +
            ...
            `regression — agent reached terminal without firing request_user_input. See ` +
            `.context/attachments/pasted_text_2026-05-06_10-25-23.txt.\n` +
