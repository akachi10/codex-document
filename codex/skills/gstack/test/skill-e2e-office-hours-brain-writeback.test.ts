...
Read pitch.md — that's a founder pitch coming to office hours. Select Startup Mode. Skip any request_user_input — this is non-interactive; auto-decide the recommended option for any question.
...
This is a test of the brain-writeback path. Do NOT skip the gbrain save step under any circumstance — the runtime guard ("skip if gbrain not on PATH") does NOT apply here because gbrain IS available. Do NOT explore gbrain --help; follow the SAVE_RESULTS template's exact CLI shape. If you encounter any request_user_input, auto-decide recommended.`,
