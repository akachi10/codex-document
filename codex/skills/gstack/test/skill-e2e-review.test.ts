...
Instead of running ~/.codex/skills/gstack/bin/gstack-review-read, run this mock: ${mockBinDir}/gstack-review-read
...
Write the dashboard output to ${dashDir}/dashboard-output.md`,
    ...
    // Should NOT contain request_user_input gate (no blocking)
    const gateQuestions = result.toolCalls.filter(tc =>
      tc.tool === 'mcp__conductor__request_user_input' ||
      (tc.tool === 'request_user_input')
    );
