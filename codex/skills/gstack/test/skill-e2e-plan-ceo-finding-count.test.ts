/**
 * /plan-ceo-review per-finding request_user_input count (periodic, paid, real-PTY).
 *
 * Asserts the load-bearing rule "One issue = one request_user_input call" by
...
import {
  ...
  type request_user_inputFingerprint,
 ...
 */
function pickSkipInterview(fp: request_user_inputFingerprint): number {
...
describeE2E('/plan-ceo-review per-finding request_user_input count (periodic)', () => {
  test(
    `5-finding plan emits ${FLOOR_DISTINCT}-${CEILING_DISTINCT} review-phase request_user_inputs`,
    async () => {
      try {
  ...
  test(
    `paired-finding positive control: ${N_PAIRED} related findings produce ${FLOOR_PAIRED}-${CEILING_PAIRED} request_user_inputs`,
    async () => {
      try {
