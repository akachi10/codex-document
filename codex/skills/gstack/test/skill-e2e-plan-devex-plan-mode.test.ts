...
describeE2E('plan-devex-review plan-mode smoke (gate)', () => {
  ...
  test('request_user_input surfaces when --disallowedTools request_user_input is set', async () => {
    ...
    if (
      ...
      throw new Error(
        `plan-devex-review request_user_input-blocked regression: outcome=${obs.outcome}\n` +
          `summary: ${obs.summary}\n` +
          `elapsed: ${obs.elapsedMs}ms\n` +
          `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
      );
    ...
    if (obs.outcome === 'plan_ready') {
      if (!obs.planFile || !planFileHasDecisionsSection(obs.planFile)) {
        throw new Error(
          `plan-devex-review request_user_input-blocked regression: plan_ready without a "## Decisions" section in ${obs.planFile ?? '<no plan file detected>'} — Step 0 was silently skipped.\n` +
            `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
        );
      }
    }
