/**
 * request_user_input Format resolver — gate-tier assertions on the generated
...
describe('generateAskUserFormat — v1.7.0.0 Pros/Cons format', () => {
  ...
  test('includes request_user_input Format header', () => {
...
describe('generateAskUserFormat — runtime-failure prose fallback', () => {
  ...
  test('documents the unavailable/failed subsection', () => {
    expect(out).toMatch(/When request_user_input is unavailable or a call fails/i);
  });
