 ...
 *   - Registry completeness (every request_user_input pattern has an id)
...
// -----------------------------------------------------------------------
// Registry completeness — warn about SKILL.md.tmpl request_user_input calls
// that don't appear to map to any registry entry.
//
// This is NOT a strict CI failure. Many request_user_input invocations are
...
describe('request_user_input template coverage (informational)', () => {
  test('count of templates using request_user_input is non-trivial', () => {
    const templates = findAllTemplates();
    const usingAsk = templates.filter((p) =>
      fs.readFileSync(p, 'utf-8').includes('request_user_input'),
    );
    // At the time of writing, ~35 templates reference request_user_input.
  ...
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        ...
        localSkillRoot: '.codex/skills/gstack',
        binDir: '~/.codex/skills/gstack/bin',
        browseDir: '~/.codex/skills/gstack/browse/dist',
        designDir: '~/.codex/skills/gstack/design/dist',
      },
      preambleTier: 2,
    };
