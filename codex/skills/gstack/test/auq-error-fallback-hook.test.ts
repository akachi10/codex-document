...
describe('directiveFor — per-session-kind instruction', () => {
  ...
  test('headless directive BLOCKs', () => {
    expect(directiveFor('headless')).toMatch(/BLOCKED — request_user_input unavailable/);
  });
...
describe('hook integration — invoked as PostToolUse', () => {
  test('error result + headless env → injects BLOCK directive', () => {
    const out = runHook(
      { tool_name: 'mcp__conductor__request_user_input', tool_response: '[Tool result missing due to internal error]' },
      { GSTACK_HEADLESS: '1' },
    );
    expect(out.additionalContext).toMatch(/BLOCKED — request_user_input unavailable/);
  });

  test('error result + interactive env → injects prose-triad directive', () => {
    const out = runHook(
      { tool_name: 'request_user_input', tool_response: null },
  ...
  test('error result + spawned env → injects auto-choose directive', () => {
    const out = runHook(
      { tool_name: 'request_user_input', tool_response: { is_error: true } },
      { OPENCLAW_SESSION: '1' },
    );
  ...
  test('SUCCESSFUL answer → no injection (inert on real answers)', () => {
    const out = runHook(
      { tool_name: 'request_user_input', tool_response: { answers: [{ option_label: 'A' }] } },
      { GSTACK_HEADLESS: '1' },
    );
