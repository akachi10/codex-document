 ...
 *   spec-dedupe-matches     — merge-with-or-file-new request_user_input for matches
...
describe('/spec --dedupe gh failure handling', () => {
  ...
  test('matches surface as request_user_input with merge-or-file-new options', () => {
...
describe('/spec --execute dirty-worktree gate', () => {
  ...
  test('offers 3-option request_user_input (continue / stash / cancel)', () => {
    expect(TMPL).toMatch(/Continue.*uncommitted/i);
    expect(TMPL).toMatch(/Stash and restore/i);
    expect(TMPL).toMatch(/Cancel spawn/i);
  });
  test('TOCTOU re-check fires after request_user_input answer', () => {
