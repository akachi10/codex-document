 ...
 * skipped Step 0 entirely and went straight to leave Plan mode. The original
 * failure had the assistant read a diff, write a plan with two issues, and
 * call leave Plan mode without ever firing request_user_input — the user had to
    ...
    if (obs.outcome !== 'asked') {
      const diagnosis =
        obs.outcome === 'plan_ready'
          ? `'plan_ready' first means the agent skipped Step 0 entirely and went straight to leave Plan mode without asking.`
          : obs.outcome === 'timeout'
            ...
            : obs.outcome === 'silent_write'
              ? `Silent Write/Edit fired to an unsanctioned path before any request_user_input — also a Step 0 skip.`
      ...
      throw new Error(
        `plan-ceo-review smoke FAILED: outcome=${obs.outcome}\n` +
          ...
          `and the "One issue = one request_user_input call" rule under "CRITICAL RULE — ` +
