/**
 * request_user_input format-compliance gate (gate, paid, SDK capture).
 *
 * Asserts: /plan-ceo-review's first request_user_input (Step 0F mode selection) is a
 ...
 * writes the verbatim request_user_input it would have shown to a file — clean text,
...
describeE2E('request_user_input format compliance (gate)', () => {
  test(
    "/plan-ceo-review's first request_user_input is a compliant decision brief (7/7 + substance)",
    async () => {
      ...
      if (!text.trim()) {
        throw new Error('No request_user_input captured — the skill never reached its mode-selection question.');
      }
      ...
      if (fmt.missing.length > 0) {
        throw new Error(
          `request_user_input missing ${fmt.missing.length} mandated format element(s): ` +
            `${fmt.missing.join(', ')}\n--- captured AUQ ---\n${text}`,
        );
      }
