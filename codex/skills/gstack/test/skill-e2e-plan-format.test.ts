/**
 * request_user_input format regression test for /plan-ceo-review and /plan-eng-review.
 *
 * Context: a user on Opus 4.7 reported the RECOMMENDATION line and the
 * `Completeness: N/10` per-option score stopped appearing on request_user_input
 * prompts. This test captures the agent's request_user_input output verbatim
 * and asserts the format rule is applied.
 *
 * Capture shape: `claude -p` sessions inside this harness do not have the
 * request_user_input MCP tool wired. We instruct the agent to write the verbatim
 * request_user_input text it would have made to $OUT_FILE instead of calling
...
// Regex predicates applied to captured request_user_input content.
...
// request_user_input content to a file instead of calling a tool.
function captureInstruction(outFile: string): string {
  return `Write the verbatim text of every request_user_input you would have made to ${outFile} (one question per session, full text including options and recommendation line). Do NOT call any tool to ask the user. Do NOT paraphrase — include the exact prose you would have shown. This is a format-capture test, not an interactive session.`;
}
...
Proceed to Step 0F (Mode Selection). This is where the skill presents 4 mode options (SCOPE EXPANSION, SELECTIVE EXPANSION, HOLD SCOPE, SCOPE REDUCTION) to the user via request_user_input. These options differ in kind (review posture), not in coverage.
...
Proceed to Step 0C-bis (Implementation Alternatives / Approach Menu). This is where the skill generates 2-3 approaches (minimal viable vs ideal architecture) and presents them via request_user_input. These options differ in coverage (complete vs shortcut), so Completeness: N/10 applies.
...
// --- Case 3: plan-eng-review coverage-differentiated per-issue request_user_input ---
...
During your review (Section 3 Test Review is the natural place), generate ONE request_user_input about test coverage depth where the options are clearly coverage-differentiated. For example:
...
// --- Case 4: plan-eng-review kind-differentiated per-issue request_user_input ---
...
During your review (Section 1 Architecture), generate ONE request_user_input about an architectural choice where the options differ in kind, not in coverage. For example, "should we use Redis or Postgres for the cache layer?" — the options are different kinds of systems with different tradeoffs, not more-or-less-complete versions of the same thing.
