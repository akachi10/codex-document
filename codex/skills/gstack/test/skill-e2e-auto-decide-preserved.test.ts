 ...
 * to prefer mcp__*__request_user_input variants and fall back to plan-file
 ...
 *
 * Spawn:
 *   claude --permission-mode plan --disallowedTools request_user_input
 *   /plan-ceo-review
 *
 * Expected:
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when request_user_input is --disallowedTools', async () => {
    ...
    try {
      ...
      if (obs.outcome === 'asked') {
        throw new Error(
          `AUTO_DECIDE regression: the model surfaced an request_user_input despite the user's never-ask preference.\n` +
            `summary: ${obs.summary}\n` +
