...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .codex/skills/gstack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // ~/.codex/skills/gstack/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    // Extract request_user_input format instructions from generated SKILL.md
    const skillMd = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    const aqStart = skillMd.indexOf('## request_user_input Format');
...
Since this is non-interactive, DO NOT actually call request_user_input. Instead, write the EXACT text you would display to the user (the full request_user_input content) to the file: ${outputPath}
...
Do NOT use request_user_input — run Quick tier directly.
...
Choose HOLD SCOPE mode. Skip any request_user_input calls — this is non-interactive.
...
Choose SELECTIVE EXPANSION mode. Skip any request_user_input calls — this is non-interactive.
...
Proceed directly to the full review. Skip any request_user_input calls — this is non-interactive.
Write your complete review directly to ${planDir}/review-output.md
...
Run /retro for the last 7 days of this git repo. Skip any request_user_input calls — this is non-interactive.
...
IMPORTANT:
- Do NOT use request_user_input — auto-approve everything or skip if unsure.
...
You are running /gstack-upgrade standalone. The gstack installation is at ./.codex/skills/gstack (local-git type — it has a .git directory with an origin remote).
...
Skip any request_user_input calls — auto-approve the upgrade. Write a summary of what you did to stdout.

IMPORTANT: The install directory is at ./.codex/skills/gstack — use that exact path.`,
...
Skip research — work from your design knowledge. Skip the font preview page. Skip any request_user_input calls — this is non-interactive. Accept your first design system proposal.
...
DO research what's out there before proposing — search for civic tech and government data platform designs. Skip the font preview page. Skip any request_user_input calls — this is non-interactive.
...
Skip research. Skip font preview. Skip any request_user_input calls — this is non-interactive.`,
      workingDirectory: designDir,
      maxTurns: 20,
      timeout: 360_000,
...
Skip research. Skip any request_user_input calls — this is non-interactive. Generate the font and color preview page but write it to ./design-preview.html instead of /tmp/ (do NOT run the open command). Then write DESIGN.md.`,
      workingDirectory: designDir,
      maxTurns: 20,
      timeout: 360_000,
...
Skip the preamble bash block. Skip any request_user_input calls — this is non-interactive. Rate each design dimension 0-10 and explain what would make it a 10. Then EDIT plan.md to add the missing design decisions (interaction state table, empty states, responsive behavior, etc.).
...
Skip the preamble bash block. Skip any request_user_input calls — this is non-interactive. Write your findings directly to stdout.
...
Review the site at ${serverUrl}. Use --quick mode. Skip any request_user_input calls — this is non-interactive. Fix up to 3 issues max. Write your report to ./design-audit.md.`,
...
Do NOT use request_user_input — for any request_user_input prompts, choose the RECOMMENDED option automatically.
