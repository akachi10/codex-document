 ...
 * one request_user_input and then batches the remaining findings into a
 * single "## Decisions to confirm" plan write + leave Plan mode.
 *
...
describeE2E('/plan-eng-review multi-finding batching regression (periodic)', () => {
  test(
    `4-finding plan emits >= ${FLOOR} review-phase request_user_inputs (no batching)`,
    async () => {
      try {
        ...
        if (obs.reviewCount < FLOOR) {
          throw new Error(
            `BATCHING REGRESSION: reviewCount=${obs.reviewCount} < FLOOR=${FLOOR}.\n` +
              ...
              `a single plan write + leave Plan mode instead of asking one per finding.\n` +
