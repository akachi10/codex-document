...
describe('runAgentSdkTest — canUseTool extension', () => {
  ...
  test('request_user_input is auto-added to allowedTools when canUseTool is supplied', async () => {
    freshSem();
    const stub: StubConfig = {
      streams: [[systemInit(), assistantTurn([{ type: 'text', text: 'ok' }]), resultSuccess()]],
      calls: [],
    ...
    await runAgentSdkTest({
      ...BASE_OPTS,
      allowedTools: ['Read', 'Grep'], // explicitly omits request_user_input
      queryProvider: makeStubProvider(stub),
      canUseTool: async (_toolName, input) => ({ behavior: 'allow', updatedInput: input }),
    });
    const opts = stub.calls[0]!.options!;
    expect(opts.allowedTools).toContain('request_user_input');
    expect(opts.tools).toContain('request_user_input');
  });

  test('request_user_input is NOT auto-added when canUseTool is absent', async () => {
    freshSem();
    const stub: StubConfig = {
      streams: [[systemInit(), assistantTurn([{ type: 'text', text: 'ok' }]), resultSuccess()]],
      calls: [],
    ...
    const opts = stub.calls[0]!.options!;
    expect(opts.allowedTools).not.toContain('request_user_input');
  });

  test('passThroughNonrequest_user_input helper returns allow+updatedInput', async () => {
    const { passThroughNonrequest_user_input } = await import('../test/helpers/agent-sdk-runner');
    const result = passThroughNonrequest_user_input('Read', { file_path: '/tmp/x' });
