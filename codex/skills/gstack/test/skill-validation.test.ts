...
describe('v0.4.1 preamble features', () => {
  // Tier 1 skills have core preamble only (no request_user_input format)
  const tier1Skills = ['SKILL.md', 'browse/SKILL.md', 'setup-browser-cookies/SKILL.md', 'benchmark/SKILL.md'];

  // Tier 2+ skills have request_user_input format with RECOMMENDATION
  ...
  for (const skill of tier2PlusSkills) {
    test(`${skill} contains request_user_input Pros/Cons format`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill), 'utf-8');
...
describe('Codex skill', () => {
  ...
  // request_user_input. The CODEX_PLAN_REVIEW content renders into each skill's
