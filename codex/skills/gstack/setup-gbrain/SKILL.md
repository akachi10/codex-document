...
dead Postgres URL). Fire a targeted request_user_input BEFORE Step 2:
...
`broken-config`, fire the same request_user_input again (the user picks again).
...
## Step 2: Pick a path (request_user_input)
...
Do NOT silently pick; fire the request_user_input.
...
Ask the D17 tier prompt via request_user_input: "Which Supabase tier?" Present
...
List orgs, pick one (request_user_input if multiple):
...
- `deny` → do nothing.
- `unset` → request_user_input: "How should `<normalized-remote>` interact with
  gbrain?"
...
Separate request_user_input: "Also sync your gstack artifacts (CEO plans,
...
Otherwise (the "many transcripts on disk" path): request_user_input with
...
(local engines are inherently single-tenant). No request_user_input.
...
trust policy question via request_user_input:
...
For each orphan, request_user_input per project: "Delete orphan project
