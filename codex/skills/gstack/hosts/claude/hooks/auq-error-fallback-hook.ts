#!/usr/bin/env bun
/**
 * PostToolUse hook for request_user_input — runtime reliability layer for the
 * AUQ-failure prose fallback (OV3:B).
 *
 * When an request_user_input call comes back as an ERROR / missing result (the
 ...
 * On a SUCCESSFUL request_user_input (a real answer), the hook defers (no output).
 *
 * Triggered by ~/.claude/settings.json (registered by `setup` next to
 * question-log-hook):
 *   PostToolUse matcher "(request_user_input|mcp__.*__request_user_input)"
 *
 * Invariants:
 ...
 * request_user_input (which carries the user's choice) is never misread as failure.
 */
...
export function directiveFor(kind: 'spawned' | 'headless' | 'interactive'): string {
  const lead =
    'The request_user_input call did not return a usable answer (error / missing result). ' +
    'Per the request_user_input failure-fallback rule: ';
  switch (kind) {
    case 'spawned':
      return (
        lead +
    ...
    case 'headless':
      return (
        lead +
        'SESSION_KIND=headless — report `BLOCKED — request_user_input unavailable` and stop; no human can answer.'
      );
    case 'interactive':
    default:
      return (
...
async function main(): Promise<void> {
  ...
  if (toolName !== 'request_user_input' && !/^mcp__.+__request_user_input$/.test(toolName)) {
    return defer();
  }
