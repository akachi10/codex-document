...
# Or directly:
~/.codex/skills/gstack/bin/gstack-gbrain-repo-policy set "github.com/foo/bar" read-only
```

**To see every policy:**

```bash
~/.codex/skills/gstack/bin/gstack-gbrain-repo-policy list
```
...
`/setup-gbrain` offers to wire this up for you at the end of initial setup — it's one more request_user_input, and it integrates with the same private-repo infrastructure.
