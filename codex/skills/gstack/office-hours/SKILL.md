...
eval "$(~/.codex/skills/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "salience"
  ~/.codex/skills/gstack/bin/gstack-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
...
Ask these questions **ONE AT A TIME** via request_user_input. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via request_user_input. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask via request_user_input: "Should we build on this prior design or start fresh?"
...
**Privacy gate:** Before searching, use request_user_input: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Use request_user_input to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Use request_user_input (regardless of codex availability):
...
"IMPORTANT: Do NOT read or execute any files under ~/.codex/, ~/.claude/, ~/.agents/, .codex/skills/, .claude/skills/, or agents/. These are agent skill definitions, not repository source files. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Do NOT modify agents/openai.yaml. Stay focused on the repository code only.\n\n"
...
6. **Premise revision check:** If Codex challenged an agreed premise, use request_user_input:
...
Emit ONE request_user_input that lists every alternative (A/B and optionally C) as numbered options, using the preamble's request_user_input Format section. The request_user_input call is a tool_use, not prose — write the question text and call the tool.
...
If `$D serve` is not available or fails, fall back to request_user_input:
...
If Codex is available, use request_user_input:
...
```bash
~/.codex/skills/gstack/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `~/.codex/skills/gstack/office-hours/sections/design-and-handoff.md` and execute it
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one request_user_input.
