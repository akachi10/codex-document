...
eval "$(~/.codex/skills/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "brand"
  ~/.codex/skills/gstack/bin/gstack-brain-cache get brand --project "$SLUG" 2>/dev/null || printf '_(no brand digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
...
### 0D. Focus Areas
request_user_input: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
path; use this for the request_user_input URL AND as the base for the reload
...
**PRIMARY WAIT: request_user_input with board URL**

After the board is serving, use request_user_input to wait for the user. Include the
...
**Do NOT use request_user_input to ask which variant the user prefers.** The comparison
board IS the chooser. request_user_input is just the blocking wait mechanism.

**After the user responds to request_user_input:**
...
5. Reload the board in the user's browser (same tab) — the URL is per-board
   ...
   daemon.
6. The board auto-refreshes. **request_user_input again** with the same board URL to
   wait for the next round of feedback. Repeat until `feedback.json` appears.

**If `NO_FEEDBACK_FILE`:** The user typed their preferences directly in the
request_user_input response instead of using the board. Use their text response
as the feedback.
...
Use request_user_input to verify before proceeding.
...
**Do NOT use request_user_input to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use request_user_input to confirm you understood the feedback correctly, never to re-ask what they chose.
...
5. request_user_input if there's a genuine design choice to resolve
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.codex/skills/gstack/plan-design-review/sections/review-sections.md` and execute it
