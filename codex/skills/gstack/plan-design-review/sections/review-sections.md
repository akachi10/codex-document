...
**STOP.** request_user_input once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** request_user_input once per issue. Do NOT batch. Recommend + WHY.
...
Each decision = one request_user_input with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.
...
request_user_input: "The review passes changed [list major design changes]. Want me to regenerate mockups to reflect the updated plan? This ensures the visual reference matches what we're actually building."
...
Follow the request_user_input format from the Preamble above. Additional rules for plan design reviews:
...
* **Zero findings:** if a section has zero findings, state "No issues, moving on" and proceed. Otherwise, use request_user_input for each gap — a gap with an "obvious fix" is still a gap and still needs user approval before any change lands in the plan.
* **NEVER use request_user_input to ask which variant the user prefers.** Always create a comparison board first (`$D compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use request_user_input ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual request_user_input. Never batch TODOs — one per question. Never silently skip this step.
...
If any request_user_input goes unanswered, note it here. Never silently default to an option.
...
eval "$(~/.codex/skills/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  ~/.codex/skills/gstack/bin/gstack-brain-cache invalidate brand --project "$SLUG" 2>/dev/null || true
```
