...
If `_POLICY == "unset"` AND `_HASH != "local"`, request_user_input per the
...
not `--full`, request_user_input via the format in the preamble:
...
`PAGES > 0`, request_user_input via the format in the preamble:
