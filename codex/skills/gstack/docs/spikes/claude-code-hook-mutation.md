...
Can a PreToolUse hook on `request_user_input` actually substitute the user's
...
To cover both native + MCP `request_user_input`:
```json
"matcher": "(request_user_input|mcp__.*__request_user_input)"
```

Conductor disables native `request_user_input` via `--disallowedTools` and
routes through `mcp__conductor__request_user_input` — the MCP suffix is
...
- If a user has THEIR own hook that also returns `updatedInput` on
  request_user_input, the merge order is undefined.
...
(`hosts/claude/hooks/auq-error-fallback-hook.ts`) that, when an request_user_input
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.codex/skills/gstack/hosts/claude/hooks/question-preference-hook",
            "timeout": 5
          }
        ]
      }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.codex/skills/gstack/hosts/claude/hooks/question-log-hook",
            "timeout": 5
          }
        ]
      }
...
1. **Recommended-option parsing scope.** D2 says parse `(recommended)`
   ...
   request_user_input Format. Implementation will need to walk `tool_input.
