...
request_user_input-shaped events from it for `gstack-codex-session-import`?
...
## Critical finding: Codex has no `request_user_input` tool

Codex doesn't surface request_user_input as a tool call in `response_item`
stream. Gstack skills running on Codex emit request_user_input-shaped
Decision Briefs as plain prose inside `agent_message` events (the
`request_user_input Format` from preamble). The user's answer comes back in
...
- **Claude Code:** hook captures structured `tool_input`/`tool_output`
  for `request_user_input`. Question + options + answer all separated.
...
2. **Pattern fallback.** When no marker, parse for:
   - `D<N> — <title>` line (D-number from request_user_input Format)
   - `Recommendation: ...` line
