...
| [`/plan-tune`](#plan-tune) | **Question Tuner** | Self-tune request_user_input sensitivity per question. Mark questions as never-ask, always-ask, or only-for-one-way. |
...
It works like `/plan-ceo-review` and `/plan-eng-review` — interactive, one issue at a time, with the **STOP + request_user_input** pattern. It rates each design dimension 0-10, explains what a 10 looks like, then edits the plan to get there. The rating drives the work: rate low = lots of fixes, rate high = quick pass.
...
6. request_user_input: what needs to change?
...
Keep gstack current with one command. It detects your install type (global at `~/.codex/skills/gstack` vs vendored in your project at `.codex/skills/gstack`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.
...
For each score < 7, uses request_user_input to present the issue with recommended fix.
