...
1. **"Substrate" was false.** The plan wired 5 skills to read the profile on preamble, but request_user_input is a prompt convention, not middleware. Agents can silently skip the instructions. You cannot reliably build auto-decide on top of an unenforceable convention. Without a typed question registry that every request_user_input routes through, the substrate claim is marketing.
...
1. **Typed question registry** (`scripts/question-registry.ts`). Every request_user_input gstack uses is declared with `{id, skill, category, door_type, options[], signal_key?}`. Schema-governed.
2. **CI enforcement.** Lint test (gate tier) asserts every request_user_input pattern in SKILL.md.tmpl files has a matching registry entry. Fails CI on drift, renames, or duplicates.
...
~/.gstack/projects/{SLUG}/
  question-log.jsonl                # every request_user_input, append-only, registry-validated
...
2. Before each request_user_input:
...
### Core schema
- `scripts/question-registry.ts` — typed registry. Seeded from audit of all SKILL.md.tmpl request_user_input invocations.
...
- Cons: Requires auditing every existing request_user_input in gstack — substantial up-front work.
...
- Every request_user_input invocation in every SKILL.md.tmpl has a registry entry. CI lint enforces.
...
1. Audit every `request_user_input` invocation in every gstack SKILL.md.tmpl. Build initial `scripts/question-registry.ts` with IDs, categories, door_types, options. This is the foundation; everything else sits on it.
