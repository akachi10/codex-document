...
- **ELI10 as a new resolver file (`scripts/resolvers/eli10-writing.ts`).** Codex Pass 1 caught the conflict with existing "smart 16-year-old" framing in preamble's request_user_input Format section. Fold into existing preamble instead.
...
- **Acceptance criterion "≤ 12 request_user_input prompts per /autoplan."** Removed from V1 — that target requires the pacing overhaul now in V1.1.
...
Generated SKILL.md body (static Markdown, baked at gen-skill-docs):
  - request_user_input Format section (existing V0)
...
- Pros: Modular.
- Cons (Codex #7): Conflicts with existing "smart 16-year-old" framing in preamble's request_user_input Format section. Two sources of truth.

**Extend preamble (chosen):** Writing Style section added to `scripts/resolvers/preamble.ts` directly below request_user_input Format.
