...
Note: The request_user_input happens through the Claude subprocess (which has access to
request_user_input), not through the extension UI (which doesn't have an ask-user primitive).
...
- [ ] Implement special telemetry (request_user_input on detection)
