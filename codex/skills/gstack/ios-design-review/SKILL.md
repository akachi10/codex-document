...
~/.codex/skills/gstack/bin/gstack-timeline-log '{"skill":"ios-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/gstack/bin/gstack-question-log '{"skill":"ios-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
4. Use request_user_input for any score < 7 — present the issue with
...
| Screenshot is black/blank | App may be in foreground but not rendering; request_user_input to confirm the app is in the expected state |
| 10 screens, but ground-truth screen list said 12 | request_user_input: were 2 hidden behind state we haven't triggered? |
