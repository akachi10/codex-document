...
`~/.codex/skills/gstack/ios-qa/templates/` (or the worktree's
...
4. Otherwise, replace the file outright with the new template (after
   request_user_input if the diff is non-trivial).
...
| Symptom | Action |
|---|---|
| Swift compile fails after regen | Revert via `git restore` + request_user_input: surface the compile error |
