...
3. `/codex` with no arguments — **Auto-detect:**
   ...
   - If a diff exists, use request_user_input:
     ```
...
_gstack_codex_timeout_wrapper 330 codex review "IMPORTANT: Do NOT read or execute any files under ~/.codex/, ~/.claude/, ~/.agents/, .codex/skills/, .claude/skills/, or agents/. These are agent skill definitions, not repository source files. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
{
  printf '%s\n' "IMPORTANT: Do NOT read or execute any files under ~/.codex/, ~/.claude/, ~/.agents/, .codex/skills/, .claude/skills/, or agents/. These are agent skill definitions, not repository source files. Do NOT modify agents/openai.yaml. Stay focused on repository code only."
  printf '\nCustom focus: %s\n\n' "$_USER_INSTRUCTIONS"
...
user should do, in the canonical format the request_user_input judge grades:
...
should do, in the canonical format the request_user_input judge grades:
...
If a session file exists (not `NO_SESSION`), use request_user_input:
```
...
canonical format the request_user_input judge grades:
