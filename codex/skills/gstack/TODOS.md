...
**What:** Every generated SKILL.md hardcodes the literal `~/.codex/skills/gstack/...`
...
pointing at a non-existent `~/.codex/skills/gstack/` path — failing **silently, at
...
- **Test conflict (verified):** `test/gen-skill-docs.test.ts:1942` and the sibling
  ship assertion currently *assert* generated Claude output `.toContain('~/.codex/skills/gstack')`
...
and stays inline. Move the rarely-needed preamble REFERENCE blocks (the request_user_input
...
**What:** `/plan-devex-review` has a "Plan Mode Handshake" section at the top that contradicts the preamble's "Skill Invocation During Plan Mode" contract (which says request_user_input satisfies plan mode's end-of-turn requirement). The handshake forces an extra exit-plan-mode step that no other interactive review skill needs. `/plan-ceo-review`, `/plan-eng-review`, `/plan-design-review` all run fine in plan mode without it.
...
**Context:** The handshake section in `plan-devex-review/SKILL.md.tmpl` says it's needed because plan mode's "this supersedes any other instructions" warning could otherwise bypass the skill's per-finding STOP gates. But the same warning exists for the other review skills, and they all work fine because request_user_input satisfies the end-of-turn contract.
...
## P2: Per-finding request_user_input count assertion for /plan-ceo-review

**What:** PTY E2E test that drives /plan-ceo-review through Step 0 with a stable fixture diff containing N known findings, asserts that exactly N distinct request_user_inputs fire (one per finding) before plan_ready.

**Why:** The skill template repeats "One issue = one request_user_input call. Never combine multiple issues into one question." at every review checkpoint. No test enforces it. The current `skill-e2e-plan-ceo-plan-mode.test.ts` smoke (post-v1.21.1.0) only catches "agent skipped Step 0 entirely." Batching findings into one question slips through silently.
...
**Context:** The PTY harness (`runPlanSkillObservation`) returns at first terminal outcome — for V2 we need a streaming variant that counts request_user_inputs across the whole session up to `plan_ready`. Probably a new helper alongside `runPlanSkillObservation`.
...
**Why:** Without env honoring, the v1.21.1.0 plan-ceo-review smoke is still flaky on machines with `question_tuning: true` set in YAML. AUTO_DECIDE preferences would skip the rendered request_user_input list, masking the regression we want to catch.
...
**What:** Design and implement a structural forcing function that catches when a skill mandates per-issue request_user_input but the model silently substitutes batch-synthesis. Candidate mechanisms: question-count assertion (skill declares expected question count in frontmatter; post-run audit logs if model fired <N), typed question templates (skill hands the model pre-built request_user_input payloads rather than prose instructions), or a canUseTool-based post-run audit that compares declared-gates-fired vs expected.

**Why:** The authoritative "Skill Invocation During Plan Mode" rule (hoisted to preamble position 1) tells the model request_user_input satisfies plan mode's end-of-turn requirement. That fixes plan-mode entry, but NOT the broader class of failures: the model silently substitutes batch-synthesis for STOP-Ask loops whenever the skill's interactive contract collides with any other rule surface (auto mode, tool-count anxiety, cognitive load). Without structural enforcement, every skill with STOP-per-issue contracts remains vulnerable.
...
**What:** The mutating-flow sibling of `/scrape` (Phase 2b). `/automate <intent>` codifies form fills, click sequences, and multi-step interactions into permanent browser-skills. Reuses Phase 2a's skillify machinery (`/skillify` is shared) and the D3 atomic-write helper. Adds: per-mutating-step UNTRUSTED-wrapped summary + `request_user_input` confirmation gate when running non-codified (codified skills run unattended after the initial human approval). Defaults to `trusted: false` per Phase 1 — env-scrubbed spawn, scoped-token capability, no admin scope.
...
adaptations as **advisory annotations on request_user_input recommendations**
...
**What:** After a few runs, check index.md for user's usual tier pick, skip the request_user_input.
...
**Context:** Would require logging choices (e.g., append to a JSONL file when request_user_input resolves), parsing them, and displaying trends. Similar pattern to eval persistence.
...
- **Added `test/skill-e2e-brain-privacy-gate.test.ts`** — periodic-tier E2E (~$0.30-$0.50/run). Stages a fake `gbrain` on PATH + `gbrain_sync_mode_prompted=false` in config, runs a real skill via `runAgentSdkTest`, intercepts tool-use via `canUseTool`, and asserts the preamble fires the 3-option privacy request_user_input with canonical prose ("publish session memory" / "artifact" / "decline"). Second test asserts the gate is silent when `prompted=true` (idempotency-within-session).
