...
Review which request_user_input prompts fire across gstack skills, set per-question preferences
...
10. **"Turn it off" / "disable"** → `~/.codex/skills/gstack/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `~/.codex/skills/gstack/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(~/.codex/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
   echo "QUESTION_TUNING: $_QT"
   echo "CONTRIBUTOR: $_CONTRIB"
   ```

2. request_user_input (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   ...
   > request_user_input outcome locally to
...
1. Ask FIVE one-per-dimension declaration questions via individual
   request_user_input calls (one at a time). Use plain English, no jargon:
...
"
echo '---DISTILL---'
~/.codex/skills/gstack/bin/gstack-distill-free-text --status
```
...
1. Show the proposals:
   ```bash
   ~/.codex/skills/gstack/bin/gstack-distill-apply --list
   ```

2. For each unapplied proposal, present it as a numbered item and use
   request_user_input (one per call, per skill convention). Show:
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.codex/skills/gstack/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ~/.codex/skills/gstack/bin/gstack-distill-apply --proposal N
   ```
...
1. Run distill:
   ```bash
   ~/.codex/skills/gstack/bin/gstack-distill-free-text
   ```
...
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using gstack — `Other` responses on request_user_input feed
   this loop."
...
```bash
~/.codex/skills/gstack/bin/gstack-distill-free-text --background
```
