...
<PR body from above>
PR_BODY_EOF
~/.codex/skills/gstack/bin/gstack-redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | ~/.codex/skills/gstack/bin/gstack-redact --repo-visibility "$REDACT_VIS" --json
```
