...
manually. The only difference: intermediate request_user_input calls are auto-decided using
...
feasibility blocker (not a preference), the request_user_input framing explicitly
...
request_user_input: you do, using the 6 principles, instead of the user.
...
Override: every request_user_input → auto-decide using the 6 principles.

**Override rules:**
- Mode selection: SELECTIVE EXPANSION
- Premises: accept reasonable ones (P6), challenge only clearly wrong ones
- **GATE: Present premises to user for confirmation** — this is the ONE request_user_input
...
Present as a message, then use request_user_input:
...
- **Two gates.** The non-auto-decided request_user_inputs are: (1) premise confirmation in Phase 1, and (2) User Challenges — when both models agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
