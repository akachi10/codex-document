...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If the user hasn't provided enough context, ask ONE question at a time via request_user_input.
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use request_user_input:
   ```
...
5. **If the fix touches >5 files:** Use request_user_input to flag the blast radius:
   ```
...
- **If fix touches >5 files → request_user_input** about blast radius before proceeding.
- **Completion status:**
