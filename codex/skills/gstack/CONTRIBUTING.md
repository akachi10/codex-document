...
gstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.codex/skills/gstack/` (your global install). But when you're developing gstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
4. **Symlink your fork into the project where you hit the bug:**
   ...
   ln -sfn /path/to/your/gstack-fork .codex/skills/gstack
   cd .codex/skills/gstack && bun install && bun run build && ./setup
   ```
...
install (`~/.codex/skills/gstack`), guarded so it never touches a symlinked or
...
| Paths | `~/.codex/skills/gstack` vs `$GSTACK_ROOT` |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.codex/skills/gstack`. `bin/dev-teardown` restores the global one.
...
ln -sfn /path/to/your/gstack-checkout .codex/skills/gstack
```
...
Remove the project-local symlink. Claude Code falls back to `~/.codex/skills/gstack/`:

```bash
rm .codex/skills/gstack
```
...
```bash
cd .codex/skills/gstack && ./setup --no-prefix   # switch to /qa, /ship
cd .codex/skills/gstack && ./setup --prefix       # switch to /gstack-qa, /gstack-ship
```
