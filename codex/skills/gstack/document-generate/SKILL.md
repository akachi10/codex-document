...
2. Use request_user_input to confirm scope and ask about documentation target:
...
If the plan has more than 5 documents to create, use request_user_input to confirm before proceeding.
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  ~/.codex/skills/gstack/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
