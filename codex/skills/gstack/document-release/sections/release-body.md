...
For each risky or questionable update identified in Step 2, use request_user_input with:
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/ship` from the
   ...
   rewriting history.
4. If an entry looks wrong or incomplete, use request_user_input — do NOT silently fix it.
...
- Auto-fix minor voice adjustments. Use request_user_input if a rewrite would alter meaning.
...
5. Flag any contradictions between documents. Auto-fix clear factual inconsistencies (e.g., a
   version mismatch). Use request_user_input for narrative contradictions.
...
2. **Items needing description updates:** If a TODO references files or components that were
   significantly changed, its description may be stale. Use request_user_input to confirm whether
   the TODO should be updated, completed, or left as-is.

3. **New deferred work:** Check the diff for `TODO`, `FIXME`, `HACK`, and `XXX` comments. For
   each one that represents meaningful deferred work (not a trivial inline note), use
   request_user_input to ask whether it should be captured in TODOS.md.
...
3. **If VERSION was NOT bumped:** Use request_user_input:
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   d. **If there are significant uncovered changes:** Use request_user_input explaining what the
...
~/.codex/skills/gstack/bin/gstack-redact --from-file /tmp/gstack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
