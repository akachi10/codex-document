...
1. `docs/api/` - 接口定义
2. `docs/testing/` 下与本任务相关的测试文档（如有）——代码必须通过这些测试
3. `docs/ops/status.md` - **环境状态** - 确认开发环境可用
4. `docs/ops/` 下的权威运维总览文档（各项目命名不一，常见为 `ops.md`）——服务启动指南
...
TDD 规则权威定义在 `~/.codex/skills/test-driven-development/SKILL.md`（项目补丁 `~/.codex/skills/tdd-amendment-bug-fix/SKILL.md`——bug-fix 黄测 + DB 断言）——**backend-dev 必须应用**，铁律"NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST"、红旗清单、例外豁免规则全以 skill 为准，本文件不复述。

### 逐级 TDD（后端硬约束，保留）

**有数据流的功能，TDD 的红→绿逐级推进**：每完成一级当场自验（灰盒＝调真实接口 / 查真实 DB 观测中间态；冒烟＝关键路径最小可用性检查），明确「这级绿了」还是「挂在哪一级」，绿了再上一级。交付**必须逐级报绿 / 报挂**，不许只说"做完了"。逐级的作用是把故障钉在产生它的那一级，不累积到最后（原理同 `~/.codex/skills/layered-diagnosis/SKILL.md`）。

后端两级，自底向上：
1. **数据层** —— 真落库 / 查得到（**psql 实查确认**，不是看代码以为落了）→ 绿
2. **接口层** —— ApiResponse 符合期望（**curl / 测试实调确认**）→ 绿

**排查 BUG 时（强制）**：接到"现象不对但不知道为什么"的故障，**先应用 `~/.codex/skills/layered-diagnosis/SKILL.md` 分层定位**——按数据→接口→渲染→视觉自底向上验，每层留证据，钉死故障在哪一层，再只改那一层。禁止凭直觉猜原因（"大概是缓存""可能连错库"）就动手改代码。
