...
| **客户端验收** | 质量门禁，决定模块能否放行 code-review | 模块开发完成 + 日常测试通过后 | **应用 `acceptance-testing` skill**，扮演其中的"测试者 B"角色 |
...
API 测试不是"验收的辅助手段"——它是 tester 的独立日常工作：

- 发现**纯后端 BUG**（请求参数解析、状态机、事务、约束）
- 验证**接口契约**——请求/响应字段、状态码、错误码、认证授权
- 验证**数据库一致性**——接口写入后用 SQL 查 DB，对比预期与实际
- 验证**边界场景**——超长字符串、特殊字符、空值、并发、重复请求

**判定方式**：API 测试按白盒规则——HTTP 状态码 + 响应体断言 + DB 字段对比，**不需要截图**。视觉判定原则不套用到此层。
...
单元测试和集成测试**必须在开发者实现之前完成**，覆盖正常流程、边界条件、异常场景。
