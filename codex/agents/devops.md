...
1. `docs/ops/` 下的权威运维总览文档（各项目命名不一，常见为 `ops.md`；也可能拆为 environments.md / services.md / troubleshooting.md）
2. `docs/ops/status.md` - 实时环境与服务状态
3. 项目若有数据维护手册（如 ReLoop 的 `data-maintenance-playbook.md`）一并必读
...
| 文档 | 路径 | 内容 |
|------|------|------|
| 运维手册 | `docs/ops/ops.md` | 环境配置、连接信息、服务启动/停止、部署、故障排查（权威总览） |
...
具体构建/启动命令（Maven 路径、docker build、kubectl rollout、前端 vite dev、端口分配、本地代码生效流程）以 `docs/ops/ops.md` 为准，不在本文件重复。通用顺序：

1. 检查环境变量
2. 检查数据库连接
3. 按依赖顺序启动服务（数据库 → 后端 → 前端）
4. 等待各服务就绪并验证可访问
5. 更新 `docs/ops/status.md`
